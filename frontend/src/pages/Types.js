...
import { DataTable, SearchAndFilters, ConfirmDialog, useNotification, LoadingSpinner } from "../components/common";
...
const Types = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      minWidth: 120,
      renderCell: (params) => (
        <Chip label={params.value} color={params.value === "Película" ? "primary" : "secondary"} size="small" />
      ),
    },
    {
      field: "format",
      ...
      renderCell: (params) => {
        ...
        return (
          <Tooltip title={platforms.join(", ")}>
            <Chip label={`${platforms.length} plataformas`} size="small" color="info" />
          </Tooltip>
        );
      },
    },
        ...
        <Box sx={{ display: "flex", gap: 0.5 }}>
          ...
          {params.row.isActive ? (
            <Tooltip title="Desactivar">
              <IconButton size="small" onClick={() => handleDeactivate(params.row)} color="warning">
                <DeactivateIcon fontSize="small" />
              </IconButton>
            </Tooltip>
          ) : (
            <Tooltip title="Restaurar">
              <IconButton size="small" onClick={() => handleRestore(params.row)} color="success">
  ...
  const handleFormSubmit = async (formData) => {
    try {
      ...
      showNotification(`Error al ${formMode === "create" ? "crear" : "actualizar"} el tipo`, "error");
    } finally {
      setLoading(false);
    }
  };
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3 }}>
        ...
        <Box sx={{ display: "flex", gap: 1 }}>
          <Button variant="outlined" startIcon={<RefreshIcon />} onClick={loadTypes} disabled={loading}>
            Actualizar
          </Button>
      ...
      >
        <DialogTitle>{formMode === "create" ? "Crear Nuevo Tipo" : "Editar Tipo"}</DialogTitle>
        <DialogContent>
          <TypeForm
            type={selectedType}
      ...
      {/* View Dialog */}
      <Dialog open={viewDialog.open} onClose={() => setViewDialog({ open: false, type: null })} maxWidth="sm" fullWidth>
